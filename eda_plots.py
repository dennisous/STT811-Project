...
def delay_histogram(df):
    ...
    ax.axvline(15, color="#E64B35", linestyle="--", linewidth=1.4, label="15-min FAA threshold")
...
def delay_by_hour(df):
    ...
    fig, ax = plt.subplots(figsize=(4.2, 3.0))
    ax.plot(hour_delay.index, hour_delay.values,
            marker="o", linewidth=2, color="#3C5488", markersize=5)
...
def correlations(df):
    numeric_cols = [
        "DEP_DELAY", "DEP_DEL15", "DISTANCE", "DEP_HOUR", "DAY_OF_WEEK", "MONTH",
        "WEATHER_DELAY", "NAS_DELAY", "SECURITY_DELAY", "LATE_AIRCRAFT_DELAY",
        "CARRIER_DELAY", "PREV_FLIGHT_DELAY", "HAS_PREV_FLIGHT",
        "AIRPORT_TRAFFIC", "IS_HOLIDAY", "DELAY_CLASS",
    ]
    avail = [c for c in numeric_cols if c in df.columns]
    corr = df[avail].corr()
    target = (corr["DELAY_CLASS"]
              .drop(["DELAY_CLASS", "DEP_DELAY", "DEP_DEL15"])
              .sort_values(ascending=False))

    fig, ax = plt.subplots(figsize=(4.5, 3.6))
    colors = ["#00A087" if v > 0 else "#E64B35" for v in target.values]
    bars = ax.barh(range(len(target)), target.values,
...
def carrier_panel():
    cols = [
        "OP_UNIQUE_CARRIER", "DEP_DEL15", "DEP_DELAY", "CANCELLED",
    ...
    ax.set_title("Delay Rate by Carrier (% flights $\\geq$15 min late)", fontweight="bold")
    ...
    med = d.median()
    colors = ["#00A087" if v < 0 else "#3C5488" if v < med else "#E64B35" for v in d.values]
    ...
    for bar, v in zip(bars, d.values):
        ax.text(v + (0.15 if v >= 0 else -0.15),
                bar.get_y() + bar.get_height() / 2,
                f"{v:.1f}", va="center",
    ...
    plot_df.columns = ["Carrier", "Weather", "NAS (Air Traffic)", "Security", "Late Aircraft"]
    plot_df.plot(kind="barh", stacked=True, ax=ax, width=0.8,
    ...
    for bar, v in zip(bars, d.values):
        ax.text(v + 0.02, bar.get_y() + bar.get_height() / 2,
                f"{v:.2f}M", va="center", fontsize=11)
    ax.grid(axis="x", color="#e5e5e5", linewidth=0.8, zorder=0)
    ax.set_axisbelow(True)
...
def main():
    cols = [
        "DEP_DELAY", "DEP_DEL15", "DISTANCE", "DEP_HOUR", "DAY_OF_WEEK", "MONTH",
        "DELAY_CLASS", "WEATHER_DELAY", "NAS_DELAY", "SECURITY_DELAY",
        "LATE_AIRCRAFT_DELAY", "CARRIER_DELAY", "PREV_FLIGHT_DELAY",
        "HAS_PREV_FLIGHT", "AIRPORT_TRAFFIC", "IS_HOLIDAY",
    ]
